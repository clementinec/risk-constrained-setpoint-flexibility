...
def validate_matrix_and_manifest(
    ...
    if plan.get("schema_version") != EXPECTED_PLAN_SCHEMA or plan.get("all_gates_passed") is not True:
    ...
    for _index, row in frame.iterrows():
        ...
        if sizing != {"mode": "accepted_denver", "source_idf_sha256": EXPECTED_IDF_SHA256}:
...
def freeze_inputs(args: argparse.Namespace, preflight: Preflight) -> FrozenInputs:
    ...
    frozen_contracts = {
        ...
        files[args.matrix]: (preflight.plan["matrices"][args.matrix.stem]["csv_sha256"]),
